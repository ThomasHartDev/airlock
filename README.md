...
For the strongest isolation tier, `runInDocker` runs the same source in a disposable container with no network, a read-only rootfs, dropped caps, and no host env. Grants must be JSON-serializable. Requires Docker (default image `node:20-alpine`).
